...
Wed 3:00 PM"), GoldenClaw back-solves your cap from what it independently
...
is roughly stable. Re-calibrate whenever the panel and GoldenClaw disagree —
`goldenclaw calibrate` is cheap and self-correcting.
"""
...
CALIBRATION = Path.home() / ".config" / "goldenclaw" / "calibration.json"
...
def night_budget_guard(reserve_pct=15.0):
    ...
    if not s.get("calibrated"):
        return True, ("not calibrated — run `goldenclaw calibrate` so the night "
