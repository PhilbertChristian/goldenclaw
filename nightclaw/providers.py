...
NightClaw therefore supports two kinds of provider, and never blurs them:
  ...
  manual  — no local telemetry. You read that provider's own panel and tell
            NightClaw the percentage. It stores the reading with a timestamp
...
STORE = Path.home() / ".config" / "nightclaw" / "calibration.json"
...
def save_manual_reading(name, percent_used, reset_spec, plan=None, now=None):
    ...
    store["providers"][name] = {
        ...
        "note": ("A point-in-time reading you transcribed, not a measurement. "
                 "NightClaw counts down to the reset and reports the reading's "
